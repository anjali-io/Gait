...
def cuda_dist(x, y, metric='euc'):
    ...
    is_3d = len(x.size()) == 3
    num_bin = x.size(2) if is_3d else 1
    if not is_3d:
        x = x.unsqueeze(-1)
        y = y.unsqueeze(-1)
    ...
    # Initialize distance matrix on CPU to prevent GPU OOM
    ...
    for i in range(num_bin):
        ...
        for start in range(0, n_x, chunk_size):
            end = min(start + chunk_size, n_x)
            _x_chunk = x[start:end, :, i]
            
            if metric == 'cos':
                chunk_dist = torch.matmul(_x_chunk, _y.transpose(0, 1))
            else:
                _dist = torch.sum(_x_chunk ** 2, 1).unsqueeze(1) + torch.sum(_y ** 2, 1).unsqueeze(0) - 2 * torch.matmul(_x_chunk, _y.transpose(0, 1))
                chunk_dist = torch.sqrt(F.relu(_dist))
            
            # Accumulate on CPU
            dist[start:end, :] += chunk_dist.cpu()
