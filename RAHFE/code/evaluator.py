...
def evaluate_real_scene_with_distractor(data, dataset, metric='euc'):
    """
    Official GREW evaluation that merges the distractor set into the gallery.

    Sequence type convention (set by rearrange_GREW.py):
        '01'  -> gallery (seq_01 per subject)
        '02'  -> probe   (seq_02 per subject, used as local probe)
        '03'  -> external probe (original GREW test probe, anonymous)
        '04'  -> distractor sequences (233,857 sequences, anonymous subjects)

    Gallery = seq_type '01' + seq_type '04'  (6,000 + 233,857 = ~239,857 total)
    Probe   = seq_type '02'
    """
    msg_mgr = get_msg_mgr()
    feature, label, seq_type = data['embeddings'], data['labels'], data['types']
    ...
    # ── Build gallery: real gallery (01) + distractors (04) ──────────────────
    gallery_mask = np.isin(seq_type, ['01', '04'])
    gallery_x = feature[gallery_mask, :]
    gallery_y = label[gallery_mask]

    # ── Build probe: seq_02 subjects ─────────────────────────────────────────
    probe_mask = np.isin(seq_type, ['02'])
    probe_x = feature[probe_mask, :]
    probe_y = label[probe_mask]

    if probe_x.shape[0] == 0:
        msg_mgr.log_info('[Distractor Eval] No probe sequences found (seq_type==02). '
                         'Check your dataset configuration.')
        return {}

    msg_mgr.log_info(f'[Distractor Eval] Gallery size : {gallery_x.shape[0]:,}  '
                     f'(real={np.sum(seq_type[gallery_mask] == "01"):,}, '
                     f'distractor={np.sum(seq_type[gallery_mask] == "04"):,})')
    msg_mgr.log_info(f'[Distractor Eval] Probe size   : {probe_x.shape[0]:,}')
    ...
    msg_mgr.log_info('==Rank-1 (with distractor)==')
    msg_mgr.log_info('%.3f' % np.mean(acc[0]))
    msg_mgr.log_info('==Rank-5 (with distractor)==')
    msg_mgr.log_info('%.3f' % np.mean(acc[4]))
    msg_mgr.log_info('==Rank-10 (with distractor)==')
    msg_mgr.log_info('%.3f' % np.mean(acc[9]))
    msg_mgr.log_info('==Rank-20 (with distractor)==')
    msg_mgr.log_info('%.3f' % np.mean(acc[19]))

    return {
        "scalar/test_accuracy/Rank-1":  float(np.mean(acc[0])),
        "scalar/test_accuracy/Rank-5":  float(np.mean(acc[4])),
        "scalar/test_accuracy/Rank-10": float(np.mean(acc[9])),
        "scalar/test_accuracy/Rank-20": float(np.mean(acc[19])),
    }
