...
def load_data(dataset_path, resolution, dataset, pid_num, pid_shuffle, cache=True):
    seq_dir, view, seq_type, label = [], [], [], []

    for _label in sorted(os.listdir(dataset_path), key=lambda x: int(x)):
        ...
        for _seq_type in sorted(os.listdir(label_path)):
            ...
            for _view in sorted(os.listdir(seq_type_path)):
                seq_path = osp.join(seq_type_path, _view)

                frames = [f for f in os.listdir(seq_path) if f.endswith('.png')]
                if len(frames) >= 10:
